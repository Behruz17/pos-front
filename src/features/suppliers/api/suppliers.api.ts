...
import type {
  ...
  TSupplierStatsResponse,

} from '../model/suppliers.types'

import { supplierDtoSchema, supplierOperationsSchema, supplierStatsSchema } from '../model/suppliers.schemas'
...
const suppliersApi = baseApi.injectEndpoints({
  ...
  endpoints: (build) => ({
    ...
    getSupplierStats: build.query<TSupplierStatsResponse, { warehouse_id?: number; start_date?: string; end_date?: string }>({

      query: (params) => ({

        url: '/suppliers/stats/payment-summary',
      ...
      transformResponse: (response) => supplierStatsSchema.parse(response),
