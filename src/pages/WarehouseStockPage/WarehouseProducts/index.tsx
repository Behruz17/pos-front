...
import { SupplierStatsModal } from '@/widgets/modals/SupplierStatsModal';
...
import { getCurrencyDisplay } from '@/shared/utils/currencyUtils'
...
export const WarehouseProducts = ({
  ...
  // Group suppliers balance by currency
  const getBalancesByCurrency = useMemo(() => {
    if (!suppliersData?.suppliers) return {}
    
    return suppliersData.suppliers.reduce((acc: Record<string, number>, supplier: { balance?: string | number; currency?: string | null }) => {
  ...
  const uniqueCurrencies = useMemo(() => {
    if (!suppliersData?.suppliers) return []
    const currencies = new Set(suppliersData.suppliers.map(s => s.currency || 'som'))
    return Array.from(currencies).sort()
  }, [suppliersData])

  // Filter suppliers by currency
  const filteredSuppliers = useMemo(() => {
    if (!suppliersData?.suppliers) return []
    if (currencyFilter === 'all') return suppliersData.suppliers
    return suppliersData.suppliers.filter(supplier => 
      (supplier.currency || 'som') === currencyFilter
    )
    ...
    <div className="space-y-6">
      ...
      {tab === 'suppliers' && !selectedSupplier && (
        ...
        <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="rounded-xl border border-slate-300 pl-3 pr-8 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 appearance-none bg-white"
          ...
          ) : suppliersData?.suppliers && filteredSuppliers.length > 0 ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
                <div className="flex justify-between items-start">
                  <div className="text-blue-800">
                    <span className="font-semibold">Общий баланс всех поставщиков по валютам:</span>
                    <div className="mt-2 space-y-1">
                      {Object.entries(getBalancesByCurrency).map(([currency, balance]) => (
                      ...
                      {Object.keys(getBalancesByCurrency).length === 0 && (
                        <span className="text-sm text-blue-600">Нет данных о балансах</span>
                      )}
                    </div>
                  </div>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    ...
                    {filteredSuppliers
                      ...
                      .map((supplier: { id: number; name: string; phone: string; balance: number; currency?: string | null }) => (
                      ...
                      >
                        ...
                        <Td className="text-slate-600 text-sm">
                          ...
                          {getCurrencyDisplay(supplier.currency)}
              ...
              <div className="sm:hidden space-y-3">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-3">
                  <div className="flex justify-between items-start">
                    <div className="text-blue-800">
                      <span className="font-semibold">Общий баланс по валютам:</span>
                      <div className="mt-2 space-y-1">
                        {Object.entries(getBalancesByCurrency).map(([currency, balance]) => (
                          <div key={currency} className="flex items-center gap-2">
                            <span className="text-lg font-bold">
                ...
                {filteredSuppliers
                  ...
                  >
                    ...
                    <div className="mt-2 flex justify-between items-center">
                      ...
                      <span className="font-semibold text-slate-700">{supplier.balance.toFixed(2)} {getCurrencyDisplay(supplier.currency)}</span>
                    </div>
                    ...
                    <div className="mt-2 flex justify-between items-center">
                      ...
                      <span className="text-slate-600">{getCurrencyDisplay(supplier.currency)}</span>
              ...
              <div className="text-green-800">
                ...
                <span className="text-xl font-bold">
                  ...
                  {suppliersData?.suppliers
                    ...
                    ?.find((s: { id: number; balance: number; currency?: string | null }) => s.id === selectedSupplier.id)?.balance

                    ?.toFixed(2) || '0.00'} {getCurrencyDisplay(suppliersData?.suppliers?.find((s: { id: number; balance: number; currency?: string | null }) => s.id === selectedSupplier.id)?.currency)}
                    ...
                    {receiptsData.operations.map((operation: { id: number; type: string; date: string; sum: number }, index: number) => (
                      ...
                      >
                        ...
                        <Td right className="font-semibold text-slate-700">
                          ...
                          {operation.sum.toFixed(2)} {getCurrencyDisplay(suppliersData?.suppliers?.find((s: { id: number; balance: number; currency?: string | null }) => s.id === selectedSupplier.id)?.currency)}
